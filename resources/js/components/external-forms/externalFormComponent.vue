        ...
        <div v-if="page_error!=null" class="card border-0 shadow-lg">
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <span class="font-weight-bold h4 m-0 float-right"> {{form_name}} </span>
                ...
                <div>
                                 ...
                                 <div class="form-group">
                                    ...
                                    <div class="input-group-append">
                                        <button :class="'btn btn-'+app_color" type="button" @click="findStudent(inputs[itm['name']] = itm['value'])">
                                            <span v-if="student_verified == false">Find</span>
                                            <span v-else="student_verified == true">Verified</span>
                                        </button>
                                    </div>
                                    </div>
                                    <div v-if="errors && errors['inputs.'+itm['name']]" class="badge badge-danger">{{ errors['inputs.'+itm['name']][0] }}</div>
                    ...
                    <div class="row p-3">
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-6 text-right">
                            <button :class="'btn btn-'+app_color" v-if="toType(pages[cur_page  + 1]) !== 'undefined'&&cur_page!=max_page" @click="change_page(cur_page + 1)">Next <i class="fas fa-arrow-circle-right"></i></button>
...
export default {
    ...
    data () {
        return {
            ...
            // form_name:form_name,
            formm:form,
            has_staff_checking:has_staff_checking,
            fullname: '',
            cur_page : 0,
            pages : {},
    ...
    computed:{
        max_page(){
            if(typeof this.has_staff_checking != 'undefined'){
                if(this.has_staff_checking==1){
        ...
        form_name(){
            if(this.formm == 'complaints_and_appeals') return 'Complaints and appeals'
            if(this.formm == 'application_for_release_letter') return 'Application for release letter'
            if(this.formm == 'application_for_deferment') return 'Application for deferment suspension cancellation withdrawal'
            if(this.formm == 'student_general_request_form') return 'Student general request form'
            if(this.formm == 'qualification_request_form') return 'Qualification request form'
            if(this.formm == 'refund_request_form') return 'Refund request form'
            if(this.formm == 'critical_incident_report_form') return 'Critical incident request form'
        }
    },
    created() {
    ...
    methods: {
        ...
        saveForm () {
            ...
            var dis = this;
            var data ={'pages':this.pages,'inputs':this.inputs};
            ...
            // console.log(data);
            swal.fire({
                title: 'Submitting '+this.form_name,
            ...
            axios.post('/external-forms/save/'+this.formm,data).then(
                response=>{
                   if(response.data.status=='success'){
                       swal.fire(
                            'Saved!',
